...
- 描くものはライティングした立方体二つと、その下の床一枚、床の縁の壁である。色は主体ごとに一色で、面ごとの色はない。壁は別の一色である。透視カメラ。ビューのドラッグでプレイヤーの周りを回る
- ImGui で変えるのはクリア色、プレイヤーの位置と回転、カメラ距離、歩行をカメラのヨーに合わせる切り替え
- 主体は並びである。先頭がプレイヤー、次が相手。相手の初期位置は `(0, 0, 4)` で、ヨーはプレイヤーを向く。相手は動かない
- ビューポートにカーソルがあるとき、WASD でプレイヤーが XZ を歩く。既定の前後左右はプレイヤーのヨーで、歩行はヨーを上書きしない。切り替えるとカメラの水平ヨーになり、移動中だけヨーが移動方向を向く。動いたあとに、壁か他の主体へ重なった軸の移動だけ取り消す。Y は変えない
