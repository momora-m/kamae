...
- ImGui で変えるのはクリア色、立方体の位置と回転、カメラ距離、歩行をカメラのヨーに合わせる切り替え
- ビューポートにカーソルがあるとき、WASD で立方体が XZ を歩く。既定の前後左右は立方体のヨーで、歩行はヨーを上書きしない。切り替えるとカメラの水平ヨーになり、移動中だけヨーが移動方向を向く。立方体の底が床から出る一歩は止める。Y は変えない
