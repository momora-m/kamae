...
相手が同じ攻撃でプレイヤーを倒すこの変更は、Windows SDK がない環境で書きました。ここではコンパイルも実行もしていません。
