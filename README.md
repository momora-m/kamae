...
| Viewport | プレイヤーと相手の立方体、床、壁、各主体の残り。左ドラッグでカメラはプレイヤーの周りを回る。試走中、マウスがあるとき WASD でプレイヤーが歩き、Space で攻撃する。相手は近づき、間合いに入って 0.5 秒後に攻撃し、その後は 0.8 秒ごとです。プレイヤーは 0.4 秒ごとで、終わり 0.15 秒の Space を覚えます。攻撃したフレームだけ判定の箱が出る。壁と主体の手前で止まる。残りが 0 になると、その立方体は消える。編集中は歩かない |
