...
| パネル | 内容 |
| --- | --- |
| Viewport | プレイヤーと相手の立方体、床、壁、各主体の残り。左ドラッグでカメラはプレイヤーの周りを回る。マウスがあるとき WASD でプレイヤーが歩き、Space で攻撃する。壁と相手の手前で止まる。相手の残りが 0 になると相手は消える。相手は動かない |
...
攻撃で残りが減るこの変更は、Windows SDK がない環境で書きました。ここではコンパイルも実行もしていません。
